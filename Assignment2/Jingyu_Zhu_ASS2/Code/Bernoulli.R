...
for (i in 1:(length(data[1,])-1)){
  boundary <- (mean(data[1:50,i])+mean(data[51:100,i]))/2
  for (j in 1:100){
    if(data[j,i] >= boundary){
      tempData[j,i] <- 1
    }
  }
}
...
for(m in 1:10){
  ...
  for (i in 1:length(alpha[,1])){
    for (j in 1:length(alpha[1,])){
      if (i == 1){
        tempAlpha <- (sum(training[1:45,j])+0.1)/(50+2*0.1) #Apply Laplase Smoothing, where Epsilon is equal to 0.1
      }else{
        tempAlpha <- (sum(training[46:90,j])+0.1)/(50+2*0.1)
      }
      alpha[i,j] <- tempAlpha
    }
  }
...
for (i in 1:length(testing[,1])){
  ...
  for (j in 1:(length(testing[1,])-1)){
    if (testing[i,j] == 1){
      tempProduct1 <- tempProduct1 * alpha[1,j]
      tempProduct2 <- tempProduct2 * alpha[2,j]
    }else{
      tempProduct1 <- tempProduct1 * (1-alpha[1,j])
      tempProduct2 <- tempProduct2 * (1-alpha[2,j])
    }
  }
