rm(list = ls())
data <- read.table("http://www.cs.iit.edu/~agam/cs584/data/regression/svar-set2.dat")
...
for (i in 1:10){
  ...
  zmatrix <- matrix(rep(1, length(training[,1])*4), length(training[,1]), 4)
  testzmatrix <- matrix(rep(1, length(testing[,1])*4), length(testing[,1]), 4)
  
  zmatrix[ ,2] <- training[ ,1]
  zmatrix[ ,3] <- training[ ,1]^2
  zmatrix[ ,4] <- training[ ,1]^3
  
  testzmatrix[ ,2] <- testing[ ,1]
  testzmatrix[ ,3] <- testing[ ,1]^2
  testzmatrix[ ,4] <- testing[ ,1]^3
  ...
  trainingMSE <- sum(c(t(yHat)-ymatrix) ^ 2)/amount
  ...
  xTesting <- testing[, 1]
  yTesting <- testing[, 2]
  linearModel <- lm(yTraining~xTraining + I(xTraining^2) + I(xTraining^3))
  theta0 <- linearModel$coeff[1]
  theta1 <- linearModel$coeff[2]
  theta2 <- linearModel$coeff[3]
  theta3 <- linearModel$coeff[4]
  yHatlmTrain <- (xTraining^3)*theta3 + (xTraining^2)*theta2 + xTraining*theta1 + theta0
  yHatlmTest <- (xTesting^3)*theta3 + (xTesting^2)*theta2 + xTesting*theta1 + theta0
