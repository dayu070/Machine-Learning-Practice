...
#Apply Linear SVM with Soft Margins (plotting with the first fold)
...
u = matrix(rep(0.05, nrow(trainingLinearX)))   #Set u to be 0.05 in order to use soft margins
...
u = matrix(rep(1000, nrow(trainingLinearNotSeparableX)))  #Set u to be 1000 in order to use soft margins
...
for (k in 2:10) #10 cross validation
{
  ...
  r = 0
  l = matrix(rep(0, nrow(trainingLinearNotSeparableX)))
  u = matrix(rep(1000, nrow(trainingLinearNotSeparableX)))
